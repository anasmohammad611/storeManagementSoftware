...
conn = sqlite3.connect("/home/anas/StoreManagementSoftware/Database/store.db")
c = conn.cursor()
...
class Application:
    def __init__(self, master, *args, **kwargs):
        ...
        # components
        self.heading = Label(self.left, text="Pesit general Store", font=('arial 40 bold'), bg='white')
        ...
        self.enteride = Entry(self.left, width=23, font=('arial 18 bold'), bg='lightblue')
    ...
    def ajax(self, *args, **kwargs):
       ...
       # qStr = "CREATE trigger Oopsss after insert | update | delete}  on [table_name]  [for each row]  [trigger_body]" 
        ...
        querymsg = "SELECT max(id) FROM inventory"
        resulting = c.execute(querymsg)
        for self.k in resulting:
            j = self.k[0]
        if(int(self.get_id)<=int(j)):
            ...
            tkinter.messagebox.showinfo("Oopsss!!!","No products available with that ID")
